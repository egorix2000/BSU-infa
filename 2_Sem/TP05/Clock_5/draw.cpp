...
void DrawClock(HDC hDC, INT x, INT y, INT width, INT height, HBITMAP hBmpClock, DWORD dwROP, 
	...
	double oneSecondAngle = acos(-1) / 30;
	...
	LineTo(hDC, (width / 2)*sin(seconds * oneSecondAngle) + x + height / 2,
		-(height / 2)*cos(seconds * oneSecondAngle) + y + height / 2);
	DeleteObject(SelectObject(hDC, GetStockObject(BLACK_PEN)));

	SelectObject(hDC, CreatePen(PS_SOLID, 4, RGB(0, 0, 0)));
	MoveToEx(hDC, x + width / 2, y + height / 2, nullptr);
	LineTo(hDC, (width / 4)*sin(minutes * oneSecondAngle) + x + width / 2,
		-(height / 4)*cos(minutes * oneSecondAngle) + y + height / 2);
	DeleteObject(SelectObject(hDC, GetStockObject(BLACK_PEN)));
}
